...
func (p *SubscribeTradeClearWebSocketV2Client) Subscribe(symbol string, mode string, clientId string) {
	channel := fmt.Sprintf("trade.clearing#%s#%s", symbol, mode)
...
func (p *SubscribeTradeClearWebSocketV2Client) UnSubscribe(symbol string, mode string, clientId string) {
	channel := fmt.Sprintf("trade.clearing#%s#%s", symbol, mode)
